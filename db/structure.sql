...
CREATE INDEX "index_conversations_on_mailbox_id" ON "conversations" ("mailbox_id") /*application='SharedInbox'*/;
CREATE INDEX "index_conversations_on_customer_id" ON "conversations" ("customer_id") /*application='SharedInbox'*/;
CREATE INDEX "index_conversations_on_assignee_id" ON "conversations" ("assignee_id") /*application='SharedInbox'*/;
CREATE UNIQUE INDEX "index_conversations_on_number" ON "conversations" ("number") /*application='SharedInbox'*/;
CREATE INDEX "idx_on_mailbox_id_status_last_message_at_3ae194d729" ON "conversations" ("mailbox_id", "status", "last_message_at") /*application='SharedInbox'*/;
CREATE TABLE "messages" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "conversation_id" integer NOT NULL, "agent_id" integer, "kind" varchar NOT NULL, "message_id_header" varchar, "in_reply_to" varchar, "references_header" text, "from_email" varchar, "from_name" varchar, "to" json DEFAULT '[]' NOT NULL, "cc" json DEFAULT '[]' NOT NULL, "bcc" json DEFAULT '[]' NOT NULL, "body_text" text, "body_html" text, "status" varchar DEFAULT 'received' NOT NULL, "bounce" boolean DEFAULT FALSE NOT NULL, "auto_submitted" boolean DEFAULT FALSE NOT NULL, "sent_at" datetime(6), "created_at" datetime(6) NOT NULL, "updated_at" datetime(6) NOT NULL, CONSTRAINT "fk_rails_7f927086d2"
